...
Invariants (load-bearing — see ARCHITECTURE.md §3):
  - ``main`` has NO fact-submission tools: the orchestrator does no math and can never
    fabricate a fact.
  - ``verifier`` is read-only: bounded fact/glossary reads plus literature search.
...
ROLE_TOOLS: Dict[str, Tuple[str, ...]] = {
    "worker": ("gm_add", "gm_search", "fact_submit_batch", "fact_search", "fact_get", "fact_neighborhood", "search_arxiv_theorems"),
    ...
    "verifier": ("fact_get", "glossary_get", "search_arxiv_theorems"),
    "all": ALL_TOOLS,
}
