...
def test_http_routes_expose_only_indexed_research_graph(tmp_path: Path):
    ...
    with _env(DANUS_DASHBOARD_PROJECT=str(tmp_path), DANUS_PROJECT_DIR=None):
        ...
        assert "Target conclusions contain exploration attempts" in index.text
        script = (Path(__file__).parents[1] / "static" / "app.js").read_text(encoding="utf-8")
        assert "function renderAttemptProofStructure" in script
        assert "function renderTargetOverview" in script
        assert "function renderSharedFacts" in script
        assert "fallback / pivot between attempts" in script
